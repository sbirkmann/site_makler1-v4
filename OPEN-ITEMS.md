...
| T-15 | ~~Objektdetailseite: Metadaten hinter `</head>`~~ – **kein Fehler.** Das ist Streaming Metadata von Next.js 16: weil `generateMetadata` die Datenbank abfragt, wird die Seite sofort ausgeliefert und die Tags nachgereicht (`node_modules/next/dist/docs/01-app/03-api-reference/04-functions/generate-metadata.md`, Abschnitt „Streaming metadata"). Bots, die JavaScript ausführen (Googlebot), lesen sie korrekt; für Bots ohne JavaScript liefert Next.js sie automatisch im `<head>`. Der Lighthouse-Wert 92 ist ein Messartefakt. Abschaltbar über `htmlLimitedBots: /.*/` – nicht empfohlen, kostet TTFB | erledigt |
